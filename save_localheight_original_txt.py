...
map_images = glob.glob('E:/Spatial Computing & Informatics Laboratory/CutTextArea/dataset/weinman19-maps/D0041-5370026.tiff')
output_path='../weinman19_txt/'
...
for map_path in map_images[0:1]:
    ...
    #txt_name = '../original_os_test_txt/' + base_name[0:len(base_name) - 4] + '.txt'
    ...
    for k in range(1,num_c):
        ...
        for i in range(0, len(contours[0])):
            ...
            if i < len(contours[0]) - 1:
                new_context = new_context + str(contours[0][i][0][0].item() ) + ',' + str(contours[0][i][0][1].item() ) + ','
            else:
                new_context = new_context + str(contours[0][i][0][0].item() ) + ',' + str(contours[0][i][0][1].item())
    ...
    cv2.imwrite(output_path+'prob_' + base_name[0:len(base_name) - 5] + '.jpg', prob_map_o)
    cv2.imwrite(output_path+'cent_' + base_name[0:len(base_name) - 5] + '.jpg', center_map_o)
    cv2.imwrite(output_path+'localheight_map_' + base_name[0:len(base_name) - 5] + '.jpg', localheight_map_o)
    ...
    cv2.imwrite(output_path+'parse_result_'+base_name[0:len(base_name) - 5] + '.jpg',map_img)
    cv2.imwrite(output_path+'txt_pixel_result_' + base_name[0:len(base_name) - 5] + '.jpg', txt_pixel_result)
