...
#local height
def mean_squared_error_localheight(y_true, y_pred):
    #y_mask = y_true[:,:,:,0] #bsize, h, w, 5(m,x,y,sin,cos)
    #y_mask = K.expand_dims(y_mask, axis = -1)
    #print y_true.shape, y_pred.shape
    #y_true = y_true[:,:,:,1:]
    #y_pred = y_pred[:,:,:,1:]
    return K.sum(K.square((y_pred - y_true)),  axis= -1) # mse at each pixel location

def mean_absolute_error_mask(y_true, y_pred):

    #mapTrain_2: loss1,loss2
...
def mean_absolute_error_localheight(y_true, y_pred):
    # mapTrain_2: loss1,loss2
